...
def DNN(n_classes, n_features, hidden_size, layers,activation_type, dropout):
    ...
    first_layer = nn.Linear(n_features, hidden_size[0])
    hidden_layers = nn.ModuleList([nn.Linear(hidden_size[i], hidden_size[i+1]) for i in range(layers-1)])
    output_layer = nn.Linear(hidden_size[-1], n_classes)
    if activation_type == "relu": activation = nn.ReLU()
    elif activation_type == "tanh": activation = nn.Tanh()
    model=nn.Sequential(first_layer,activation,nn.Dropout(p=dropout[0]))
    for k in range(layers-1):                                           
        model.append(hidden_layers[k])
        model.append(activation)
        model.append(nn.Dropout(p=dropout[k+1]))
    model.append(output_layer)  
    return model

def train(model, train_loader, val_loader, num_epochs, batch_size, optimizer, criterion, save_best, scheduler):
    
    trainer = optimizer(model.parameters(), lr=0.0006793215923680178, weight_decay=0.0032071139613134457)
...
parser.add_argument('-stages', type=int, nargs='+', default=[0,2,4,7,8,9,11,12,14,15]) #0,2,4,7,8,11
...
#cuts="Bgen==23333 && Bpt>%f && Bpt<%f " % (opt.ptmin, opt.ptmax)
#cutb="((Bmass - 5.27929 ) > 0.25 &&  (Bmass - 5.27929) < 0.30) && Bpt>%f && Bpt<%f" % (opt.ptmin, opt.ptmax)
...
inputS = TFile.Open("/lstore/cms/simao/sample/BPMC_3_60_small2.root")
inputB = TFile.Open("/lstore/cms/simao/sample/BPData_3_60_small2.root")

signal = inputS.Get("ntKp")
background = inputB.Get("ntKp")
...
#print(factory.GetROCIntegral(dataloader))
#roc.SaveAs('dataset/results/ROC_ClassificationPyTorch_%s_%s_%s_%s.png' % (opt.ptmin ,opt.ptmax ,stagestring,factory.GetROCIntegral(dataloader)))
